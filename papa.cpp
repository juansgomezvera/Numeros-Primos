...
int main (){
	...
	cin >> cadena;
	if (palindroma(cadena)==true)cout >>"es palindroma"; //mandamos la palabra a la funcion palindroma y si es verdadera se imprime el mensaje.
	else cout>>"no es palindroma" // si no se imprime un mensaje diciendo que no lo es.
}